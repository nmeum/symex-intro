...
// Uncomment to enable state merging, causing a speed up by merging paths.
//#define KLEE_STATE_MERGING 1

// Whether to enable URL-safe base64.
#define BASE64URLSAFE 0
 ...
 */
static char __getsymbol(uint8_t code, bool urlsafe)
{
    if (!BASE64URLSAFE) {
        urlsafe = false;
    }
...
{
#ifdef KLEE_STATE_MERGING
    klee_open_merge();
#endif
    char ret = __getsymbol(code, urlsafe);
#ifdef KLEE_STATE_MERGING
    klee_close_merge();
#endif
    return ret;
 ...
 */
static uint8_t __getcode(char symbol)
{
    if (symbol == '/') {
        return BASE64_SLASH;
    }
...
int main(void) {
    ...
    base64_encode(input, sizeof(input), in, &in_size);

    char out[128];
    size_t out_size = sizeof(out);
    base64_decode(in, in_size, out, &out_size);

    assert(out_size == INPUT_SIZE);
    assert(memcmp(input, out, out_size) == 0);
